'use server';
export const ArtisanalPromise = async () => {
  return (
    <div className="container mx-auto rounded-lg bg-gray-100 p-2 md:p-4">
      <h2 className="mb-1 text-3xl text-orange-600 md:mb-3">Promesses:</h2>
      <ul className="list-disc pl-6 md:space-y-2 text-lg">
        <li>Premium & artisanale</li>
        <li>Fumage & marinade traditionnel</li>
        <li>A la main, détails méticuleux</li>
        <li>Elevage durables</li>
        <li>Livraison avant Noël et Pâques</li>
      </ul>
    </div>
  );
};
